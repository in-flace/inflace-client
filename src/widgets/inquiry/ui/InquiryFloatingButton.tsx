...
export function InquiryFloatingButton({
  ...
  const isSubmitting = useInquiryPanel((s) => s.isSubmitting)
  ...
  return (
    <button
      type='button'
      onClick={toggle}
      /* 전송 중에는 닫을 수 없다(toggle이 막는다). 눌러도 반응이 없으면
       * 고장으로 보이므로 버튼도 같이 비활성으로 보여준다. */
      ...
      className={cn(
        'fixed right-32 z-40 flex size-56 cursor-pointer items-center justify-center rounded-full bg-brand-tertiary text-white transition-[bottom,opacity] hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60',
